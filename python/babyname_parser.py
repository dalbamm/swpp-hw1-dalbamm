...
def check_filename_existence(func):
    ...
    def wrapper_func(*args, **kwargs):
        ...
        if not os.path.isfile(filename):
            raise BabynameFileNotFoundException("No such babyname file or directroy: {0}".format(filename))
            return
    ...
    def __init__(self, filename):
        ...
        while textline !='':
            if textline.startswith('<tr align="right"') & textline.endswith('</td>\n'):
                ...
                listmp = textline[:-2].split(',')
                tupletmp = (listmp[0],listmp[1],listmp[2])
                self.rank_to_names_tuples.append(tupletmp)
